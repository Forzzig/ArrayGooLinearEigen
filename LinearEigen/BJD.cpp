...
BJD::BJD(SparseMatrix<double>& A, SparseMatrix<double>& B, int nev, int restart, int batch, int gmres_size, int gmres_restart) 
...
void BJD::genCRS(SparseMatrix<double>& A, int* ia, int* ja, double* a) {
...
//带自动扩容
void BJD::CRSsubtrac(int*& ia, int*& ja, double*& a, int& nnz, SparseMatrix<double>& B, double eff) {
    //记录增加的数
